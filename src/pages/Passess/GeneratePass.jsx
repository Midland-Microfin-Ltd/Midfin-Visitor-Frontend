...
import { getVisitorRequests, generateVisitorPass } from "../../utilities/apiUtils/apiHelper";
...
const GeneratePass = () => {
  ...
  const fetchApprovedVisitors = async (currentPage = 0, currentRowsPerPage = 10) => {
    setLoading(true);
    try {
      ...
      if (response.success) {
        ...
        showSnackbar(response.message || "Failed to fetch approved visitors", "error");
      }
    } catch (error) {
  ...
  const handleGeneratePass = async (visitor) => {
    ...
    try {
      const response = await generateVisitorPass({ visitorId: visitor.visitorId });
  ...
  const downloadPassAsPDF = async () => {
    const passElement = document.getElementById('visitor-pass-card');
    ...
    try {
      ...
      pdf.save(`visitor-pass-${passData.visitorId}.pdf`);
      
      showSnackbar("Pass downloaded successfully!", "success");
    } catch (error) {
      console.error("Error downloading pass:", error);
      showSnackbar("Error downloading pass", "error");
    }
  };
  ...
  return (
    <MiniDrawer>
                  ...
                  <TableRow>
                    <TableCell colSpan={4} align="center" sx={{ py: 4 }}>
                      <Typography color="text.secondary">
                        {searchQuery ? "No approved visitors match your search" : "No approved visitors found"}
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
      ...
      {/* Pass Generation Dialog - Minimal with only the card */}
      <Dialog 
        open={passDialog} 
          ...
          <Box
            ...
            sx={{
              ...
              "&::before": {
                ...
                right: 0,
                height: 5,
                background: "linear-gradient(90deg, #0c2461 0%, #4a69bd 50%, #0c2461 100%)",
              }
            }}
          >
            ...
            <Box sx={{ p: 3.5, position: "relative" }}>
              ...
              <Box sx={{ 
                ...
                <Typography variant="caption" sx={{ 
                  ...
                  GID Type: {passData?.governmentId?.substring(0, 6) || '475895'}
                </Typography>
              </Box>
            ...
            <Box sx={{ 
              ...
              <Typography variant="caption" sx={{ 
                ...
                This card must be returned when exiting the premises
              </Typography>
            </Box>
          </Box>
